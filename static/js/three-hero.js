...
(function initThreeHero() {
    ...
    // ---- Tomato Body (Sphere) ----
    const tomatoGeometry = new THREE.SphereGeometry(1.2, 64, 64);

    // Create custom shader material for premium tomato look
    ...
    // Slightly squash the tomato
    const tomato = new THREE.Mesh(tomatoGeometry, tomatoMaterial);
    tomato.scale.set(1.0, 0.85, 1.0);
    scene.add(tomato);

    // ---- Tomato Dimple (top indent) ----
    const dimpleGeometry = new THREE.SphereGeometry(0.35, 32, 32);
    ...
    // ---- Stem ----
    const stemGeometry = new THREE.CylinderGeometry(0.06, 0.08, 0.4, 12);
    ...
    for (let i = 0; i < 5; i++) {
        const sepal = createSepal((Math.PI * 2 / 5) * i);
        tomato.add(sepal);
    }

    // ---- Orbiting Particles (DNA-like ring) ----
    ...
    function animate() {
        ...
        tomato.rotation.y += (targetRotY - tomato.rotation.y) * 0.03;
        tomato.rotation.x += (targetRotX - tomato.rotation.x) * 0.03;
        tomato.rotation.y += 0.003; // constant slow spin

        // Float animation
        tomato.position.y = Math.sin(time * 0.8) * 0.15;
        ...
        for (let i = 0; i < particlesCount; i++) {
            const angle = (i / particlesCount) * Math.PI * 2 + time * 0.2;
            const radius = 2.2 + Math.sin(i * 0.3 + time) * 0.3;
            pos[i * 3] = Math.cos(angle) * radius;
            pos[i * 3 + 1] = Math.sin(angle * 3 + time * 0.5) * 0.5;
        ...
        ring.material.opacity = 0.05 + Math.sin(time * 2) * 0.05;
